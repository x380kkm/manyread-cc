...
# dependencies = []
# ///
"""manyscan.lib.render.html — the self-contained interactive HTML view (sigma.js)."""
...
# Sourced from the vendored asset file (the same _ASSET_DIR the sigma libs load
# from) and inlined verbatim — the asset content IS the bootstrap string value, so
# the emitted bytes are identical to the old inline literal. Read mandatorily (no
# CDN fallback: the bootstrap is the app, not an optional vendored lib).
