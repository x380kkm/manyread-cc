...
uv run --python 3.12 ${CLAUDE_PLUGIN_ROOT}/scripts/ref.py list   [--status active]
...
4. **lifecycle / reuse** — `list` scans the resolved store's `refs_dir`, `select` prints the
   manifest + notes + file list for reuse, and `shelve`/`clear`/`keep` follow the same dated
   model as dynamic traces.
